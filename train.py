...
ap.add_argument("--model", required=False, help="model: resnet18/resnet34/resnet50/densenet121/se_resnet18/se_resnet34/se_resnet50/cbm_resnet34/efficientnet-b0/efficientnet-b1/", default='se_resnet50')
...
if model == "densenet121":
    criterion_cent = CenterLoss(num_classes=num_classes, feat_dim=model_ft.classifier.in_features, use_gpu=True)
else:
    criterion_cent = CenterLoss(num_classes=num_classes, feat_dim=model_ft.fc.in_features, use_gpu=True)

optimizer_centloss = optim.Adam(model_ft.parameters(), lr=0.001, weight_decay=1e-5)  #optim.SGD(model_ft.parameters(), lr=0.01, weight_decay=0.005, momentum= 0.9)
